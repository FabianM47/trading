...
export default function MonthlyTradesModal({
  ...
  // Trades filtern: geschlossen im ausgewählten Monat
  const monthlyTrades = trades
    .filter(t => {
      if (!t.isClosed || !t.closedAt) return false;
      const closedDate = new Date(t.closedAt);
      return closedDate.getFullYear() === month.year && closedDate.getMonth() === month.month;
    })
    .sort((a, b) => {
  ...
  const totalRealized = monthlyTrades.reduce(
          ...
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-sm text-text-secondary mt-1">
                {monthlyTrades.length} geschlossene{' '}
                {monthlyTrades.length === 1 ? 'Trade' : 'Trades'}
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="mt-4 bg-background rounded-lg p-4 border-2 border-accent/30">
            <div className="flex justify-between items-center">
              <span className="text-sm text-text-primary font-medium">
                Realisierter Gewinn im {month.label}:
              </span>
              <span
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {monthlyTrades.length === 0 ? (
            <div className="text-center py-12 text-text-secondary">
              <p className="text-lg mb-2">Keine Trades in diesem Monat</p>
              <p className="text-sm">
                In {month.label} wurden keine Trades geschlossen
              </p>
            </div>
          ) : (
                        ...
                        <div className="mt-3 pt-3 border-t border-border flex gap-3 justify-center">
                          ...
                          {onDeleteTrade && (
                            <button
                              ...
                              className="text-loss hover:text-loss-dark transition-colors p-2"
                              title="Löschen"
                            >
